...
from core.fl_coordinator import BESSFlowerClient, FLCoordinator
...
class TestEdgeGatewayModules(unittest.TestCase):
    def setUp(self):
        ...
        self.model_path = Path(__file__).resolve().parent.parent.parent / "models" / "rl_arbitrage.onnx"
    ...
    def test_fl_coordinator_key_generation_and_signatures(self):
        ...
        self.assertTrue(self.private_key_path.exists(), "La clave Ed25519 debería haberse generado.")
    ...
    def test_onnx_inference_engine_loading_and_prediction(self):
        ...
        if not self.model_path.exists():
            self.skipTest(f"Modelo ONNX de referencia no encontrado en {self.model_path}. Omitiendo test.")
            
        engine = ONNXInferenceEngine(str(self.model_path))
        
        start_time = time.perf_counter()
        action = engine.predict_action(soc=0.5, solar_radiation=600.0, historical_spread=45.0, price=50.0)
        latency_ms = (time.perf_counter() - start_time) * 1000.0
        
        # Latencia debe ser inferior a 5ms en la CPU
        print(f"[TEST] Latencia de Inferencia ONNX: {latency_ms:.3f} ms")
        self.assertLess(latency_ms, 5.0, "La latencia de inferencia ONNX supera los 5ms requeridos.")
    ...
    def test_onnx_inference_engine_safety_guardrails(self):
        ...
        setpoint, status = engine.get_dispatch_setpoint(soc=0.5, temp=46.0, solar_radiation=500.0, historical_spread=30.0, price=50.0)
        ...
        self.assertEqual(setpoint_derated, setpoint_normal * 0.5, "El derating térmico debería reducir la potencia al 50%.")
        
        # 3. Guardas de SOC límite
        # Carga bloqueada si SOC >= 95%
        setpoint_soc_high, status_soc_high = engine.validate_and_clamp(action=0, soc=0.96, temp=25.0)
    ...
    def test_hardware_registry_modbus_decoding(self):
        ...
        self.assertFalse(sch_data["is_healthy"], "El inyector no debe reportar healthy si hay disparo anti-isla.")
