...
def test_lca_engine_exports_carbon_viability_score():
    ...
    expected_metric_line = (
        f'bess_carbon_viability_score{{region="CL",site_id="test_site_cl"}} {float(score)}'
    )
...
def test_fleet_coordinator_exports_injection_kw():
    ...
    site = FleetSiteState(
        site_id="test_fleet_site", node="TestNode", soc_pct=50.0, max_power_kw=500.0
    )
    ...
    metrics_out = generate_metrics().decode("utf-8")
    expected_metric_line = (
        'bess_injection_kw_capacity{fleet_site="test_fleet_site",site_id="edge"} 400.0'
    )
    assert expected_metric_line in metrics_out
...
def test_fleet_coordinator_exports_latency():
    ...
    assert (
        'bess_fleet_latency_ms_count{operation="compute_setpoints",site_id="edge"}' in metrics_out
    )
