...
def test(name: str, passed: bool, detail: str = "", ms: float = 0.0) -> None:
...
def test_health_schema():
    ...
    test("Tiene todos los campos requeridos", not missing, f"Faltan: {missing}", ms)
    test("status ∈ {healthy, degraded}", data.get("status") in ("healthy", "degraded"))
    test("uptime_s > 0", data.get("uptime_s", 0) > 0)
    test("last_cycle > 0 (el loop está corriendo)", data.get("last_cycle", 0) > 0)
    test("compliance_score es float [0–100]",
...
def test_telemetry_is_live():
    ...
    test("Los valores cambian entre ciclos (datos vivos)", changed,
         f"SOC: {d1.get('soc_pct')} → {d2.get('soc_pct')}")
    test("soc_pct está en rango [0, 100]",
         0.0 <= float(d2.get("soc_pct", -1)) <= 100.0)
    test("timestamp_utc presente en respuesta", "timestamp_utc" in d2)
...
def test_compliance_report():
    ...
    test("Tiene 11 GAPs", data.get("gaps_checked") == 11, ms=ms)
    gaps = data.get("gaps", {})
    test("GAPs van de GAP-001 a GAP-011",
         all(f"GAP-{str(i).zfill(3)}" in gaps for i in range(1, 12)))
    test("norm_ref menciona NTSyCS", "NTSyCS" in data.get("norm_ref", ""))
    test("generated_at_utc presente", "generated_at_utc" in data)
    test("report_type = NTSyCS_COMPLIANCE_REPORT",
...
def test_fleet():
    ...
    test("n_sites = 3", summary.get("n_sites") == 3, ms=ms)
    test("total_capacity_kwh = 600", summary.get("total_capacity_kwh") == 600.0)
    test("fleet_soc_pct en rango [0, 100]",
    ...
    test("Retorna lista de 3 sitios", len(sites) == 3)
    site_ids = [s.get("site_id") for s in sites]
    test("Sitios: DEMO-CL-001/002/003 presentes",
...
def test_404():
    ...
    try:
        ...
        test("Ruta desconocida retorna 404", False, "Debería haber fallado")
    except urllib.error.HTTPError as e:
        test("Ruta desconocida retorna 404", e.code == 404, f"Got {e.code}")
    except Exception as e:
        test("404 handler activo", False, str(e))
...
def test_concurrent_load():
    ...
    test(f"Todas las {N} requests completaron", len(responses) == N,
         f"{len(errors)} errores: {errors[:2]}")
    test("Cero errores bajo carga", len(errors) == 0, f"Errores: {errors}")
    avg_ms = sum(timings) / len(timings) if timings else 0
    p99_ms = sorted(timings)[-1] if timings else 0
    test(f"Latencia promedio < 200ms (got {avg_ms:.0f}ms)", avg_ms < 200)
    test(f"P99 < 500ms (got {p99_ms:.0f}ms)", p99_ms < 500)
...
def test_compliance_violation_scenario():
    ...
    if not health_ok:
        ...
        test("violations = [] cuando compliant", report.get("violations") == [])
...
def test_prometheus_metrics():
    ...
    test("Content-Type es text/plain", "text/plain" in content_type, ms=ms)
    test("Contiene bess_cycles_total", "bess_cycles_total" in text)
    test("Contiene bess_safety_blocks_total", "bess_safety_blocks_total" in text)
    test("Contiene bess_last_soc_percent", "bess_last_soc_percent" in text)
    test("Contiene bess_gateway_info", "bess_gateway_info" in text)
