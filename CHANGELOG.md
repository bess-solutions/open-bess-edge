...
### 📚 Documentation
- **compliance**: corregir nombre a BESS LABANT (Antofagasta) (`f23f1c80`, @BESS Solutions)
...
- **server**: bypass rate limiting for health, metrics, and local loopback traffic (`efae474c`, @BESS Solutions)
...
- **ci**: make optional repo checkout resilient and fix scorecard publish token failure (`4cb64d5a`, @BESS Solutions)
- **deps**: align cryptography to >=46.0.7,<47.0.0 for flwr compatibility and update deps-audit ignore (`726fb1ec`, @BESS Solutions)
- **security**: update cryptography to >=48.0.1 to eliminate GHSA-537c-gmf6-5ccf in pip-audit (`a83ff9bc`, @BESS Solutions)
...
- **mcp**: add request_ctx compatibility alias for fastmcp server dependencies (`710a9bda`, @BESS Solutions)
...
- **mypy**: update python_version to 3.12 in pyproject.toml to resolve numpy type syntax error (`5d9c4e2f`, @BESS Solutions)
- **ci**: update PYTHON_VERSION to 3.12 to fix numpy syntax error in mypy typecheck (`2b8ea423`, @BESS Solutions)
