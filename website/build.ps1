...
# Usage: powershell -ExecutionPolicy Bypass -File .\website\build.ps1
...
Write-Host ""
Write-Host "BESSAI Website Build" -ForegroundColor Cyan
Write-Host ""
...
$srcAssets = "$root\src\assets"
if (Test-Path $srcAssets) {
  Copy-Item "$srcAssets\*" $assetsDir -Force -Recurse
  Write-Host "  [OK] assets/ copied" -ForegroundColor Green
}
else {
  Write-Host "  [SKIP] No assets/ folder found" -ForegroundColor Yellow
}
...
Write-Host "  [OK] i18n/ copied" -ForegroundColor Green
