...
|---|---|---|---|---|
| ✅ **CERRADO** | BEP-0200→0300 DRL observe→write | `main.py:Step4c` | `BESSAI_DRL_WRITE=true` implementado con doble guard | v2.16.0 |
| ✅ **CERRADO** | ONNX dummy→real (All nodes CEN V4) | `models/` | ONNX (Ridge v2, R²~0.79) entrenados con API V4 | v2.16.0 |
| ✅ **CERRADO** | DuckDB Data Warehouse | `bessai-cen-data/db` | rebuild API V4 15-min | v2.16.0 |
| ✅ **CERRADO** | GCP no documentado | `docs/SETUP_GCP.md` | Guía completa: SA, roles, topic, terraform | v2.16.0 |
| 🔵 Baja | mypy `modbus_driver.py:179` | Múltiple | type-ignore + guards | v2.16.0 |
| 🔮 Baja | MILP optimizer sin tests de integración | `milp_optimizer.py` | `tests/agents/test_milp_optimizer.py` | v2.16.0 |
...
Prometheus v2.16.0                          OK      ← localhost:9090
```
...
| Driver Modbus TCP | `src/drivers/modbus_driver.py` | **v2.16.0** | ✅ mTLS opcional (GAP-003) |
| Simulator Driver | `src/drivers/simulator_driver.py` | **v2.16.0** | ✅ 6 tags SPEC-001 normalizadas |
...
| Dashboard API | `src/interfaces/dashboard_api.py` | **v2.16.0** | ✅ Rate limiting SR 7.1 + TOTP |
| TOTP MFA | `src/interfaces/totp_auth.py` | **v2.16.0** | ✅ GAP-001 CLOSED |
| OT TLS Config | `src/interfaces/ot_tls_config.py` | **v2.16.0** | ✅ GAP-003 CLOSED |
...
| GitHub Actions CI/CD | `.github/workflows/ci.yml` | v2.16.0 | ✅ **11 jobs**: lint+typecheck+test+interop+security+terraform+helm+docker+trivy+push+**drl-market-tests** |
| **DRL CEN Env (BEP-0200 P3)** | `src/agents/bess_rl_env_cen.py` | **v1.0** | ✅ **NUEVO v2.16.0** — Gymnasium env 8-dim obs, datos V4 CEN/SEN (15-min, todas las barras) |
| **PPO Standalone Trainer** | `scripts/train_cen_standalone.py` | **v1.0** | ✅ **NUEVO v2.16.0** — PyTorch PPO sin Ray, export ONNX automático |
| **8× CEN ONNX DRL** | `models/*_drl_cen_v1.onnx` | **v1.0** | ✅ **NUEVO v2.16.0** — p95 < 0.1ms, best_reward=374 USD (Maitencillo), 2.1KB/modelo |
| **CAISOAdapter** | `src/core/market_adapter.py` | **v1.0** | ✅ **NUEVO v2.16.0** — OASIS 5-min LMP, CA (NP15/SP15/ZP26), fallback Duck Curve |
| **ERCOTAdapter** | `src/core/market_adapter.py` | **v1.0** | ✅ **NUEVO v2.16.0** — API 15-min SPP, TX (5 hubs), fallback Duck Curve |
| **ENTSOEAdapter** | `src/core/market_adapter.py` | **v1.0** | ✅ **NUEVO v2.16.0** — Transparency DA 1h, 9 zonas EU, EUR→USD, XML parsing |
...
> 🎉 **Sin bloqueadores activos** — CI/CD + Scorecard + Mutation Testing + Fuzzing operativos. OpenSSF Gold ~85% cubierto. IEC 62443 Phase 1 docs listos. Repository lint 100% limpio en v2.16.0.
...
- [ ] **Roadmap Hardware:** Cotizar formalmente y planificar adopción de pasarela edge industrial IA (ej. Winmate WNAI-E600 con Jetson AGX Orin) post-levantamiento de capital semilla.
...
v2.16.0  ████████████████████████  ✅ Fix: 18→0 errores interop · TOTP MFA · Loki SIEM
v2.16.0  ████████████████████████  ✅ mTLS OT segment (GAP-003) · stunnel · OtTlsConfig
v2.16.0  ████████████████████████  ✅ Auditoría IEC 62443 SL-2: SSP, NAD, PMS, PSIRT
v2.16.0  ████████████████████████  ✅ Rate Limiting SR 7.1 · mkdocs nav · pip-audit CI semanal
v2.16.0  ████████████████████████  ✅ Fix: markers pytest · coverage 80% CI · versión `1.4.0`→`2.4.0` en `pyproject.toml` · Dockerfile OCI label
...
| 2026-02-22 | **v2.16.0** | **378/378** | Fix: 18→0 errores interop · TOTP MFA (`totp_auth.py`) · Loki SIEM (`docker-compose.yml`) · `asyncio_mode=auto` funcional |
| 2026-02-22 | **v2.16.0** | **426/426** | GAP-003 mTLS OT: `gen_certs.sh` PKI · `stunnel-ot.conf` TLS 1.3 · `OtTlsConfig.from_env()` · `modbus_driver.py` params TLS |
| 2026-02-22 | **v2.16.0** | **426/426** | Auditoría IEC 62443 SL-2: `ssp_iec62443_sl2.md` (SSP-001) · `network_diagram.md` (NAD-001) · `patch_management_sla.md` · SECURITY.md PSIRT |
| 2026-02-22 | **v2.16.0** | **426/426** | SR 7.1 Rate Limiting (`_RateLimiter`, 300 req/min, `429+Retry-After`) · mkdocs nav SSP/NAD/PMS · `pip-audit` weekly CI · `requirements.hash.txt` scaffold |
| 2026-02-23 | **v2.16.0** | **426/426** | Fix markers pytest (`slow`, `asyncio`) · coverage 80% CI · versión `1.4.0`→`2.4.0` en `pyproject.toml` · Dockerfile OCI label `0.1.0`→`2.4.0` |
| 2026-02-23 | **v2.16.0** | **426/426** | Fix lint `simulator_driver.py` (import math redundante) · Helm `appVersion 0.7.0`→`2.4.0` · `ci.yml` Job numbering 5→10 corregido |
| 2026-02-24 | **v2.16.0** | **490/490** | Lint 360°: ruff 35/36 auto-fix · drl_agent→structlog · 20 archivos reformateados |
| 2026-02-24 | **v2.16.0-dev** | **541/547** | **Superset 6 Waves**: BENCHMARK-004/005 · 3 perfiles HW (SolarEdge/BYD/Tesla) · `lightweight_mode.py` · `alert_dispatcher.py` · 51 tests registry · early_adopters/research_topics/academic_collaboration |
| 2026-02-24 | **v2.16.0-dev** | **590/590** | **AI Environment Devoration**: Fix MARL `rewards[__all__]` · Fix C901 `handle_der_control` refactor · Fix SSL mTLS validation order · **WatchdogManager** self-healing autónomo · 19 tests nuevos |
| 2026-02-25 | **v2.16.0** | **613/613** | **Scrollytelling Landing** (React + Vite): i18n ES/EN, Lucide icons, FAQ/Features refactored, scrollytelling animations · **360° doc sync** · **BEPs 0300/0301/0302** Draft · Archivos propietarios removidos del repo público |
| 2026-02-27 | **v2.16.0** | **685/685** | **BESSAIEvolve v2** (CMAESMutator+NSGA-II+EliteArchive 24/24) · **BEP-0200 F3** env CMg real CEN (23/23) · MILP + Degradation + Watchdog · NTSyCS ComplianceStack 11 GAPs · 685 tests |
| 2026-03-02 | **v2.16.0** | **685/685** | **BEP-0300 activado**: `BESSAI_DRL_WRITE=true` opt-in con doble guard safety+clamp · **Modelos ONNX V4** (Ridge v2 R²~0.79, multi-nodo CEN 15-min) en `models/` · **DuckDB rebuild** full barras V4 · **docs/SETUP_GCP.md** guía completa |
| 2026-03-12 | **v2.16.0** | **723/723** | **🚀 BEP-0200 Phase 3 (Autopilot session)**: `bess_rl_env_cen.py` Gymnasium env CEN real · `train_cen_standalone.py` PPO PyTorch sin Ray · **8× ONNX DRL** (p95 < 0.1ms, best_reward=374 USD Maitencillo) · **`CAISOAdapter`** OASIS 5-min · **`ERCOTAdapter`** 15-min SPP · **`ENTSOEAdapter`** 9 zonas EU · **62/62 market adapter tests** · **CI Job 11** `drl-market-tests` (Py 3.12+torch-CPU) · `pyproject.toml` 2.10.0→**2.15.2** · `drl-standalone` dep group · `pilot.html` actualizado: 7 mercados, FAQ DRL, form USA+Europa · `sync_ecosystem.py --push v2.16.0` ejecutado |
