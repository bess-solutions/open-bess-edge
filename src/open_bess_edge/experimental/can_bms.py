...
logger = logging.getLogger("open_bess_edge.experimental.can_bms")
...
class DBCParser:
    ...
    def parse_string(self, content: str) -> None:
        ...
        for line in content.splitlines():
            ...
            if bo_match:
                msg_id = int(bo_match.group(1)) & 0x1FFFFFFF if int(bo_match.group(1)) & 0x80000000 else int(bo_match.group(1))
            ...
            if cm_match:
                cm_id = int(cm_match.group(1)) & 0x1FFFFFFF if int(cm_match.group(1)) & 0x80000000 else int(cm_match.group(1))
...
class IndustrialCANBMSDriver:
    ...
    def __init__(self, dbc_paths: Optional[List[str]] = None):
        ...
        base_dir = Path(__file__).resolve().parents[1] / "data" / "can_dbc"      # instalado como paquete
        if not base_dir.exists():
            base_dir = Path(__file__).resolve().parents[3] / "data" / "can_dbc"  # checkout de desarrollo
        if not dbc_paths:
            default_dbcs = [
