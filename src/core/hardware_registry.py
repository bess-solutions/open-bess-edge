...
class HardwareRegistry:
    ...
    def get_profile(cls, brand: str) -> ModbusProfile:
        ...
        if brand_upper not in cls.PROFILES:
            raise ValueError(
                f"Marca de hardware no soportada en el registro: {brand}. Homologados: {list(cls.PROFILES.keys())}"
            )
    ...
    def parse_telemetry(cls, brand: str, registers: dict[int, int]) -> dict[str, Any]:
        ...
        logger.debug(
            f"[{profile.brand}] Telemetría Modbus decodificada: SOC={soc * 100:.1f}%, Temp={temp}°C, Power={power_kw}kW"
        )
        return telemetry
