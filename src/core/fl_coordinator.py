...
class BESSFlowerClient(client.client.NumPyClient):
    ...
    def get_parameters(self, config: Dict[str, Union[bool, bytes, float, int, str]]) -> List[np.ndarray]:
        """Devuelve los pesos actuales del modelo local."""
        logger.info(f"[{self.node_id}] get_parameters solicitado por el servidor.")
        return self.parameters

    def fit(self, parameters: List[np.ndarray], config: Dict[str, Union[bool, bytes, float, int, str]]) -> Tuple[List[np.ndarray], int, Dict[str, Union[bool, bytes, float, int, str]]]:
        ...
        self.parameters = [p + 0.01 * np.random.randn(*p.shape).astype(np.float32) for p in parameters]
    ...
    def evaluate(self, parameters: List[np.ndarray], config: Dict[str, Union[bool, bytes, float, int, str]]) -> Tuple[float, int, Dict[str, Union[bool, bytes, float, int, str]]]:
...
class FLCoordinator:
    ...
    def _load_mtls_credentials(self) -> Optional[bytes]:
    ...
    def start(self, insecure: bool = False) -> bool:
        ...
        logger.info(f"Iniciando FLCoordinator para el nodo {self.node_id} -> {self.server_address}")
        ...
        if not insecure:
            ...
            if root_certs:
                ...
                logger.warning("No se encontró ca.crt en el directorio de certificados. Usando fallback...")
        ...
        try:
            ...
            if root_certs:
                ...
                logger.info(f"Simulación local FL exitosa. Ajuste firmado: {fit_metrics['signature_hex'][:16]}...")
            return True
        except Exception as e:
            logger.error(f"Error de conexión federada: {e}. Gateway operará en modo LOCAL autónomo.")
            return False

if __name__ == "__main__":
    # Prueba local rápida
    certs = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../data/firmware/certs"))
