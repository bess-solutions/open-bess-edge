...
class ONNXInferenceEngine:
    ...
    def predict_action(
        self, soc: float, solar_radiation: float, historical_spread: float, price: float
    ) -> int:
        """
    ...
    def validate_and_clamp(self, action: int, soc: float, temp: float) -> tuple[float, str]:
        ...
        if temp >= 45.0:
            logger.critical(
                f"⚠️ DISPARO TÉRMICO: Temperatura de celdas ({temp}°C) sobrepasa límite crítico (45°C). Setpoint clamped a 0 kW."
            )
        ...
        if temp > 40.0:
            ...
            logger.warning(
                f"🌡️ DERATING TÉRMICO ACTIVO: Temperatura de celdas ({temp}°C) > 40°C. Potencia limitada en un {(1 - derating_factor) * 100}%."
            )
        ...
        if soc >= 0.95 and proposed_power < 0:
            logger.warning(
                f"🔋 SOBRECARGA PROTEGIDA: SOC ({soc * 100:.1f}%) >= 95%. Bloqueada acción de carga."
            )
        ...
        if soc <= 0.05 and proposed_power > 0:
            logger.warning(
                f"🪫 DESCARGA PROFUNDA PROTEGIDA: SOC ({soc * 100:.1f}%) <= 5%. Bloqueada acción de inyección."
            )
            return 0.0, "SOC_DEEP_DISCHARGE_PROTECTION"

        return proposed_power, "NORMAL_OPERATION"

    def get_dispatch_setpoint(
        self,
        soc: float,
        temp: float,
...
if __name__ == "__main__":
    ...
    model_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../../models/rl_arbitrage.onnx")
    )
    if os.path.exists(model_path):
        engine = ONNXInferenceEngine(model_path)
        p, status = engine.get_dispatch_setpoint(
            soc=0.5, temp=25.0, solar_radiation=800.0, historical_spread=60.0, price=75.0
        )
