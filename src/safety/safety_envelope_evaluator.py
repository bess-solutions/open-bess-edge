    ...
    def evaluate_inverter_setpoint(
        ...
        if isolation_kohm < min_iso:
            return (
                False,
                0.0,
                f"BESS-GUARD-004: Interlock de aislamiento activo ({isolation_kohm:.1f}kOhm < {min_iso}kOhm)",
            )
        ...
        if p_target_kw < -max_charge_kw:
            return (
                True,
                -max_charge_kw,
                f"CLIPPED_MAX_CHARGE_C_RATE (-{max_charge_kw:.1f} kW)",
            )
        ...
        if p_target_kw > max_discharge_kw:
            return (
                True,
                max_discharge_kw,
                f"CLIPPED_MAX_DISCHARGE_C_RATE (+{max_discharge_kw:.1f} kW)",
            )
